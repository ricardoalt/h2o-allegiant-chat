...
describe("createAgent", () => {
  ...
  it("keeps artifact tools unrestricted before field brief composition; after all 4 artifacts finish, only loadSkill stays active so the model can generate a closing text without being able to call any artifact tool again", () => {
    createAgent();
    const settings = toolLoopAgentMock.mock.calls.at(-1)?.[0];
